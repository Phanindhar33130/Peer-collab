...
public class AdminController {
	...
	public ModelAndView faculty() {
	  ...
	  @PostMapping("/addStudentfile") public ResponseEntity<String>
	  addSingleStudent(
	  ...
	  @RequestParam(required = false) MultipartFile studentFile, RedirectAttributes
	  redirectAttributes) {
	  
	  try { if (sid != null && sname != null && !sname.isEmpty()) { Student student
	  = new Student(); student.setSid(sid); student.setSname(sname);
	  studentService.addStudent(student);
	  redirectAttributes.addFlashAttribute("message",
	  "Single student added successfully!"); } else if (studentFile != null &&
	  !studentFile.isEmpty()) { try (BufferedReader reader = new BufferedReader(new
	  InputStreamReader(studentFile.getInputStream()))) { String line; while ((line
	  = reader.readLine()) != null) { String[] data = line.split(","); Student
	  student = new Student(); student.setSid(sid);
	  //student.set(Integer.parseInt(data[0])); student.setSname(data[1]);
	  studentService.addStudent(student); } }
	  redirectAttributes.addFlashAttribute("message",
	  "Students uploaded successfully from file!"); } } catch (IOException e) {
